...
function wpestate_scripts() {   
    ...
    wp_enqueue_script('jquery.fancybox.pack', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox.pack.js',array('jquery'), '1.0', true); 
    wp_enqueue_script('jquery.fancybox-thumbs', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox-thumbs.js',array('jquery'), '1.0', true); 
    ...
    if($enable_stripe_status==='yes'){
        ...
        wp_enqueue_script('wpestate-stripe', get_template_directory_uri().'/js/wpestate-stripe.js',array('stripe'), '1.0', true);
    ...
    if( !is_tax() && get_post_type() === 'estate_property' ) {
        wp_enqueue_script('jquery.fancybox.pack.js', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox.pack.js',array('jquery'), '1.0', true); 
        wp_enqueue_script('jquery.fancybox-thumbs.js', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox-thumbs.js',array('jquery'), '1.0', true); 
    ...
    if($date_lang_status!='xx'){
        ...
        if( get_post_type() === 'estate_property' && ( $book_type==2 || $book_type==3 ) ){
            wp_enqueue_script('wprentals_per_hour_locale', trailingslashit( get_template_directory_uri() ).'js/locale/'.strtolower($date_lang_status).'.js',array('jquery','fullcalendar'), '1.0', true);
        }
    ...
    if( get_post_type() === 'estate_property' && !is_tax() && !is_search() && !is_tag() ){
        ...
        if($book_type==2 || $book_type==3  ){
            ...
            wp_enqueue_style('fullcalendar', trailingslashit( get_template_directory_uri() ) . 'css/fullcalendar.css');
        }
...
if ( is_page_template('user_dashboard_edit_listing.php') || is_page_template('property_list_half.php')  ){
   ...
   // print'mumummmm';
    ...
    if( 'estate_property' == get_post_type() ||  'estate_agent' == get_post_type() || is_page_template('user_dashboard_invoices.php') || basename(get_page_template()) == 'user_dashboard_my_reservations.php'  ){

        $early_discount =  floatval(get_post_meta($post->ID, 'early_bird_percent', true));
        wp_enqueue_script('wpestate_property', trailingslashit( get_template_directory_uri() ).'js/property.js',array('jquery','wpestate_control'), '1.0', true);   
    ...
    if($general_font && $general_font!='x'){
      ...
      // wp_enqueue_style( 'wpestate-opensans', "https://fonts.googleapis.com/css?family=Open+Sans:400,600,300&amp;subset=latin,latin-ext" );   
         wp_enqueue_style( 'wpestate-roboto', "https://fonts.googleapis.com/css?family=Roboto:300,400,500,700,900&display=swap&subset=latin-ext" );    
    }
    ...
    if(!is_search() && !is_404() && !is_tax() && !is_category() && !is_tag()){
        if( wpestate_check_if_admin_page($post->ID) || is_singular('estate_property') ){
                ...
                foreach($bed_types_array as $key=>$value){
                    $bed_types_array_names[sanitize_key($value)]=$value;
                }
