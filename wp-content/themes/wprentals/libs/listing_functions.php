...
if( !function_exists('estate_listing_features') ):
    function estate_listing_features($post_id){
        ...
        return $multi_return_string.'<div class="listing_detail col-md-12 feature_block_others "><div class="feature_chapter_name col-md-12">'.esc_html__('Other Features','wprentals').'</div>'.$single_return_string.'</div>';
    ...
    function wpestate_sleeping_situation($post_id){
        ...
        while($step<=$no_bedroms){
            ...
            foreach ($bed_types_array as $key_bed_types=>$label){
                ...
                if( isset( $beds_options[sanitize_key($label)][$step-1] ) &&  $beds_options[sanitize_key($label)][$step-1] >0){
                    $return_string_second.='<div class="">'. $beds_options[sanitize_key($label)][$step-1].' '.$label.'</div>';      
                }
...
if( !function_exists('wpestate_listing_terms') ):
    function wpestate_listing_terms($post_id){
        ...
        $items=array(
            'smoking_allowed'=>esc_html('Smoking Allowed','wprentals'),
            'pets_allowed'=>esc_html('Pets Allowed','wprentals'),
            'party_allowed'=>esc_html('Party Allowed','wprentals'),
            'children_allowed'=>esc_html('Children Allowed','wprentals'),
