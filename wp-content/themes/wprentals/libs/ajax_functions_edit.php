...
if( !function_exists('wpestate_ajax_front_end_submit') ):
    function wpestate_ajax_front_end_submit(){ 
        ...
        if($post_id) {
            ...
            $property_admin_area    =   '';
            rcapi_create_new_listing($new_user_id,$post_id,$submit_title,$property_description,$new_status,$api_prop_category_name,$api_prop_action_category_name,$property_city,$property_area,$wpestate_guest_no,$property_admin_area,$property_country,intval($_POST['instant_booking']) );
            ...
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    while($i< count($beds_options)){
                        ...
                        $beds_options_array[ sanitize_key($bed_types_array[$index_bed_options]) ][]= $beds_options[$i];
                        $i++;
